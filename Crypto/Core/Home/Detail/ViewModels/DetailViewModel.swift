...
class DetailViewModel: ObservableObject {
    ...
    private func createOverviewArray(coin: Coin) -> [Statistic] {
        ...
        let rank = "\(coin.rank)"
        let rankStat = Statistic(title: "Rank", value: rank, percentageChange: nil)
        
        let volume = "$" + (coin.totalVolume?.formattedWithAbbreviations() ?? "")
        let volumeStat = Statistic(title: "Volume", value: volume, percentageChange: nil)
    ...
    private func createAdditionalArray(coinDetail: CoinDetail?, coin: Coin) -> [Statistic] {
        ...
        let highStat = Statistic(title: "24h High", value: high, percentageChange: nil)
        
        let low = coin.low24H?.asCurrencyWith6Decimals() ?? "n/a"
        let lowStat = Statistic(title: "24h Low", value: low, percentageChange: nil)
        ...
        let blockStat = Statistic(title: "Block Time", value: blockTimeString, percentageChange: nil)
        
        let hashing = coinDetail?.hashingAlgorithm ?? "n/a"
        let hashStat = Statistic(title: "Hashing Algorithm", value: hashing, percentageChange: nil)
        
        let additionalArray: [Statistic] = [
            highStat, lowStat, priceChangeStat, marketCapChangeStat, blockStat, hashStat
        ]
